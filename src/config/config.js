...
const envVarsSchema = Joi.object()
  .keys({
    ...
    // Redis configuration (optional - no longer required)
    REDIS_HOST: Joi.string().optional().default('localhost').description('Redis host (deprecated)'),
    REDIS_PORT: Joi.number().optional().default(6379).description('Redis port (deprecated)'),
    REDIS_PASSWORD: Joi.string().optional().allow('').description('Redis password (deprecated)'),
    REDIS_DB: Joi.number().optional().default(0).description('Redis database number (deprecated)'),
    REDIS_TLS: Joi.string().optional().default('false').description('Enable Redis TLS (deprecated)'),
    UPSTASH_REDIS_URL: Joi.string().optional().description('Upstash Redis URL (deprecated)'),
    UPSTASH_REDIS_TOKEN: Joi.string().optional().allow('').description('Upstash Redis token (deprecated)'),
    USE_UPSTASH: Joi.string().optional().default('false').description('Use Upstash Redis service (deprecated)'),
