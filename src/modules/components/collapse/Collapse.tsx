...
import createChainedFunction from '../_utilities/createChainedFunction'
...
const collapseStyles: {
	[key in CollapseTransitionStatus]: string
} = {
	[EXITED]: 'collapse',
	[EXITING]: 'collapsing',
	[ENTERING]: 'collapsing',
...
type CollapseTransitionStatus = typeof ENTERING | typeof ENTERED | typeof EXITING | typeof EXITED
...
const Collapse = React.forwardRef<Transition<any>, CollapseProps>(
	(
		...
		const handleEntering = useMemo(
			() =>
				createChainedFunction((elem: HTMLElement) => {
					const scroll = `scroll${computedDimension[0].toUpperCase()}${computedDimension.slice(1)}` as keyof HTMLElement
				...
				{(state: TransitionStatus, innerProps: Record<string, unknown>) =>
					React.cloneElement(children, {
						...
						className: classNames(
							...
							collapseStyles[state as CollapseTransitionStatus],
