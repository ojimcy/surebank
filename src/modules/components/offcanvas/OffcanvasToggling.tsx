...
const transitionStyles: {
	[key in OffcanvasTransitionStatus]: string
} = {
	[ENTERING]: 'show',
	[ENTERED]: 'show',
}

type OffcanvasTransitionStatus = typeof ENTERING | typeof ENTERED
...
const OffcanvasToggling = React.forwardRef<Transition<any>, OffcanvasTogglingProps>(
	({ bsPrefix, className, children, ...props }, ref) => {
		...
		return (
			<TransitionWrapper ref={ref} addEndListener={transitionEndListener} {...props} childRef={(children as any).ref}>
				{(status: TransitionStatus, innerProps: Record<string, unknown>) =>
					React.cloneElement(children, {
						...
						className: classNames(
							...
							transitionStyles[status as OffcanvasTransitionStatus]
						),
					})
				}
			</TransitionWrapper>
