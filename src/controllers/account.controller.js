...
const getSelfAccountsWithBalances = catchAsync(async (req, res) => {
  const userId = req.user._id;
  const accounts = await accountService.getUserAccountsWithBalances(userId);
  res.status(httpStatus.OK).send(accounts);
});
